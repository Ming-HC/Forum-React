...
class Forumfooter extends Component {
            ...
            <section>
                <div className="page">
                    {this.state.page.map((item) => {
                        ...
                        var ele = [];
                        // if (this.state.url.split('/').length > 2) { // ??? page why use url judge
                    ...
                    <div className="outsidediv">
                        ...
                        <div className="insidediv">
                            {this.state.randompost.map(item =>
                                <a key={item.post_id} className="img_block" href={`#/forum/post/${item.post_id}`}><img src={item.imageurl ? "https://ejs.mingspace.website/image/forum/upload/" + item.imageurl : "https://ejs.mingspace.website/image/forum/demo.png"} alt={item.title} /><div className="img_tip">{item.title}</div></a>
                            )}
                        </div>
    ...
    async componentDidMount() {
        ...
        if (window.location.hash.indexOf('post') === -1 && window.location.hash.indexOf('/forum/search') === -1) {
            var url = "";
            ...
            case 2:
                ...
                break;
            case 3:
                url = window.location.hash.split("#")[1] + "/1";
                break;
            case 4:
                url = window.location.hash.split("#")[1];
                break;
