                    ...
                    <ul className={`${styles.subject} nav nav-tabs`} id={styles.myTab} role="tablist">
                        {this.state.class.map((classItem, index) =>
                            <li className='nav-item' key={classItem.class_name_eng}>
                                <a className={window.location.hash.split("#")[1] === "/forum" && index === 0 ? `nav-link ${styles.active}` : window.location.hash.split("#")[1].indexOf(classItem.class_name_eng) > -1 ? `nav-link ${styles.active}` : "nav-link"}
                    ...
                    <div id={styles.postResult}>
                        {this.state.postlist.map(Item =>
                            <div key={`post_${Item.post_id}`}>
                                ...
                                <div><img src={Item.imageurl ? "https://ejs.mingspace.website/image/forum/upload/" + Item.imageurl : "https://ejs.mingspace.website/image/forum/demo.png"} alt={Item.imageurl ? Item.imageurl : "demo.png"} /></div>
    ...
    async componentDidMount() {
        ...
        switch (window.location.hash.split("/").length) {
            case 2:
                ...
                break;
            case 3:
                postlisturl = window.location.hash.split("#")[1] + "/1";
                break;
            case 4:
                postlisturl = window.location.hash.split("#")[1];
                break;
            default:
                break;
        }
